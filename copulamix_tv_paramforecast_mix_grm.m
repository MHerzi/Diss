...
%   Ludwig-Maximilians-Univerit�t Munich
function [CopParam_pred, weights_pred] = copulamix_tv_paramforecast_mix_grm(tv_faktor, nc, family, data, P, Q, horizon, Dynamic);
...
%             nf = nf+3;
        ...
        case 't'
            ...
            elseif strcmp(Dynamic,'DCC')
                R_pred(:,:,1) = copulaParam_tv_ADCC(tv_faktor(nf+1:nf+P+Q), P, Q, data_inv, 'vorhersage', 1);
            end
            ...
            for i = 0:horizon-2
                ...
                elseif strcmp(Dynamic,'DCC')
                    sumTerm(:,:,i+2) = sumTerm(:,:,i+1) + ((1 - sum(tv_faktor(1:P)) - tv_faktor(1+P:P+Q) - tv_faktor(1+P:P)./2) ...
                        .* Rbar .* (sum(tv_faktor(1:P)) + tv_faktor(1+2*P:P+Q)).^i);
                end
                ...
                elseif strcmp(Dynamic,'DCC')
                    R_pred(:,:,i+2) = sumTerm(:,:,i+2) + R_pred(:,:,1) ...
                        .* (sum(tv_faktor(1:P)) + tv_faktor(1+2*P:P+Q)).^(i+1);
                end
            ...
            for i = 0:horizon-2
                if strcmp(Dynamic,'ADCC')
                    sumTerm(:,:,i+2) = sumTerm(:,:,i+1) + ((1 - sum(tv_faktor(1:P)) - tv_faktor(1+2*P:2*P+Q) - tv_faktor(1+P:2*P)./2) ...
                        .* Rbar .* (sum(tv_faktor(1:P)) + tv_faktor(1+2*P:2*P+Q) + tv_faktor(1+P:2*P)./2).^i);
                elseif strcmp(Dynamic,'DCC')
                    sumTerm(:,:,i+2) = sumTerm(:,:,i+1) + ((1 - sum(tv_faktor(1:P)) - tv_faktor(1+2*P:P+Q)) ...
                        .* Rbar .* (sum(tv_faktor(1:P)) + tv_faktor(1+2*P:2*P+Q)).^i);
                end
                % Für (t+1) prognostizierte Kovarianzmatrix wird mit den
                % Koeffizienten gewichtet und auf sumTerm aufaddiert
                if strcmp(Dynamic,'ADCC')
                ...
                elseif strcmp(Dynamic,'DCC')
                    R_pred(:,:,i+2) = sumTerm(:,:,i+2) + R_pred(:,:,1) ...
                        .* (sum(tv_faktor(1:P)) + tv_faktor(1+P:P+Q) + tv_faktor(1+P:P)./2).^(i+1);
                end
                % Sicherstellen dass R_pred pos. definit ist
                R_pred(:,:,i+2) = R_pred(:,:,i+2) .* (eye(s2)==0) + eye(s2);
            end
