...
%
% OUTPUTS:
%   LLF         - (negative) Log Likelihood
%   Ht          - Estimated Conditional Variance
%   likeihoods  - T by 1 vector of (negative) likelihoods
%
% COMMENTS:
...
if errortype == 1
    b0 = parameters(size(parameters,1)-(arlag+const)+1:end);
    n = length(data);
    nlag=arlag;
    if const == 1
    ...
    y = trimr(data,nlag,0);
    resid = y - x*b0;
    parameters = parameters(1:size(parameters,1)-(arlag+const));
elseif errortype == 4
    b0 = parameters(size(parameters,1)-2-(arlag+const)+1:end-2);
    n=length(data);
    nlag=arlag;
    if const == 1
        x = [ones(n,1) mlag(data,nlag)];
    ...
    y= trimr(data,nlag,0);
    resid = y - x*b0;
    parameters = [parameters(1:size(parameters,1)-(arlag+const)-2); parameters(end-1:end)];
else
    b0 = parameters(size(parameters,1)-(arlag+const):end-1);
    n = length(data);
    nlag=arlag;
    if const == 1
        x = [ones(n,1) mlag(data,nlag)];
    ...
    y = trimr(data,nlag,0);
    resid = y - x*b0;
    parameters = [parameters(1:size(parameters,1)-(arlag+const)-1); parameters(end)];
end
...
elseif errortype == 3
    nu2 = remainingparams(length(remainingparams));
elseif errortype == 4
    nu2 = remainingparams(end-1);
    lambda2 = remainingparams(end);
end
