function [Rt_pred,likelihood]=CopulaVineLL_grm_1P_forecast(phi,data,CopulaSpec)
...
if strcmp(CopulaSpec.type,'SJC')==0
    ...
    elseif (strcmp(CopulaSpec.corrspec,'DCC') == 1|| strcmp(CopulaSpec.corrspec,'TVC') == 1) && (strcmp(CopulaSpec.type,'Gaussian') == 1 ...
            ||  strcmp(CopulaSpec.type,'Clayton') ||  strcmp(CopulaSpec.type,'Rotated Clayton') || strcmp(CopulaSpec.type,'Gumbel'))
        if iscell(phi)% check if phi is cell-variable
            DynamicSpec=phi;
        else
            k=1;
            DynamicSpec=cell((.5*N*(N-1)),1);
    ...
    elseif  strcmp(CopulaSpec.corrspec,'ADCC') == 1 && (strcmp(CopulaSpec.type,'Gaussian') == 1 || ...
            strcmp(CopulaSpec.type,'Clayton') == 1 ||  strcmp(CopulaSpec.type,'Rotated Clayton') || strcmp(CopulaSpec.type,'Gumbel') == 1)
        if iscell(phi)% check if phi is cell-variable
            DynamicSpec=phi;
        else
        k=1;
    ...
    elseif strcmp(CopulaSpec.corrspec,'static') == 1 && strcmp(type,'t') == 1
        ...
        clear phi1;
        phi = crIPmat_grm(phi);
    elseif strcmp(CopulaSpec.corrspec,'AGDCC') == 1 && (strcmp(CopulaSpec.type,'Gaussian') == 1 || ...
            strcmp(CopulaSpec.type,'Clayton') == 1 ||  strcmp(CopulaSpec.type,'Rotated Clayton') || strcmp(CopulaSpec.type,'Gumbel') == 1)
        if iscell(phi)
            DynamicSpec=phi;
        else
            k=1;
            DynamicSpec=cell((.5*N*(N-1)),1);
            for i=1:6:size(phi,1)
                DynamicSpec{k,1} =phi(i:i+5);
                k=k+1;
            end
            DynamicSpec = crIPmat_grm(DynamicSpec);
        end
    elseif strcmp(CopulaSpec.corrspec,'GDCC') == 1 && (strcmp(CopulaSpec.type,'Gaussian') == 1 || ...
            strcmp(CopulaSpec.type,'Clayton') == 1 ||  strcmp(CopulaSpec.type,'Rotated Clayton') || strcmp(CopulaSpec.type,'Gumbel') == 1)
        if iscell(phi)
            DynamicSpec=phi;
        else
    ...
    if strcmp(dec,'CanonicalVine') == 1
        for j=1:N-1
            for i=1:N-j
                if j==1
                    if strcmp(CopulaSpec.corrspec,'Patton') == 1 || strcmp(CopulaSpec.corrspec,'DCC') == 1 || strcmp(CopulaSpec.corrspec,'TVC') == 1
                        [Rt{j,i}]=CopulaLL_grm_1P_forecast(DynamicSpec{j,i},[data(:,1),data(:,i+1)],CopulaSpec);
                    else
                        [Rt{j,i}]=CopulaLL_grm_1P_forecast(phi(j,i),[data(:,1),data(:,i+1)],CopulaSpec);
                    end
                else
                    if strcmp(CopulaSpec.corrspec,'Patton') == 1 || strcmp(CopulaSpec.corrspec,'DCC') == 1 || strcmp(CopulaSpec.corrspec,'TVC') ==1
                        [Rt{j,i}]=CopulaLL_grm_1P_forecast(DynamicSpec{j,i},[u{j-1,1},u{j-1,i+1}],CopulaSpec);
                    else
                        [Rt{j,i}]=CopulaLL_grm_1P_forecast(phi(j,i),[u{j-1,1},u{j-1,i+1}],CopulaSpec);
                    end
                end
            end
            if j<N-1
                for i=1:N-j
                    if j==1
                        if strcmp(CopulaSpec.corrspec,'Patton') == 1 || strcmp(CopulaSpec.corrspec,'DCC') == 1 || strcmp(CopulaSpec.corrspec,'TVC') == 1
                            u{1,i}=hfunc_grm(data(:,i+1),data(:,1),DynamicSpec{1,i},CopulaSpec);
                        else
                            u{1,i}=hfunc_grm(data(:,i+1),data(:,1),phi(1,i),CopulaSpec);
                        end
                    else
                        if strcmp(CopulaSpec.corrspec,'Patton') == 1 || strcmp(CopulaSpec.corrspec,'DCC') == 1 || strcmp(CopulaSpec.corrspec,'TVC') == 1
                            u{j,i}=hfunc_grm(u{j-1,i+1},u{j-1,1},DynamicSpec{j,i},CopulaSpec);
                        else
                            u{j,i}=hfunc_grm(u{j-1,i+1},u{j-1,1},phi(j,i),CopulaSpec);
                        end
                    end
                end
            end
    ...
    elseif strcmp(dec,'DVine')==1
        ...
        for i=1:N-1
            ...
            if strcmp(CopulaSpec.corrspec,'Patton') == 1 || strcmp(CopulaSpec.corrspec,'DCC') == 1 || strcmp(CopulaSpec.corrspec,'TVC') == 1 || strcmp(CopulaSpec.corrspec,'ADCC') == 1 ...
                ...
                [Rt{1,i},likelihood(:,countl)]=CopulaLL_grm_1P_forecast([DynamicSpec{1,i}],[data(:,i) data(:,i+1)],CopulaSpec);
                countl=countl+1;
            else
                if strcmp(type,'t') == 1 && strcmp(CopulaSpec.corrspec,'static') == 1
                    [Rt{1,i},likelihood(:,countl)]=CopulaLL_grm_1P_forecast(phi{1,i},[data(:,i) data(:,i+1)],CopulaSpec);
                    countl=countl+1;
                else
                    [Rt{1,i},likelihood(:,countl)]=CopulaLL_grm_1P_forecast(phi(1,i),[data(:,i) data(:,i+1)],CopulaSpec);
                    countl=countl+1;
        ...
        for j=2:(N-1)
            for i=1:(N-j)
                if strcmp(CopulaSpec.corrspec,'Patton') == 1 || strcmp(CopulaSpec.corrspec,'DCC') == 1 || strcmp(CopulaSpec.corrspec,'TVC') == 1 || strcmp(CopulaSpec.corrspec,'ADCC') == 1 ...
                        || strcmp(CopulaSpec.corrspec,'GDCC') == 1 || strcmp(CopulaSpec.corrspec,'AGDCC') == 1
                    [Rt{j,i},likelihood(:,countl)]=CopulaLL_grm_1P_forecast([DynamicSpec{j,i}],[u{j-1,2*i-1} u{j-1,2*i}],CopulaSpec);
                    countl=countl+1;
                else
                    if strcmp(type,'t') == 1 && strcmp(CopulaSpec.corrspec,'static') == 1
                        [Rt{j,i},likelihood(:,countl)]=CopulaLL_grm_1P_forecast(phi{j,i},[u{j-1,2*i-1} u{j-1,2*i}],CopulaSpec);
                        countl=countl+1;
                    else
                        [Rt{j,i},likelihood(:,countl)]=CopulaLL_grm_1P_forecast(phi(j,i),[u{j-1,2*i-1} u{j-1,2*i}],CopulaSpec);
                        countl=countl+1;
            ...
            if j<N-1
                ...
                if N>4
                    for i=1:(N-j-2)
                        if strcmp(CopulaSpec.corrspec,'Patton') == 1 || strcmp(CopulaSpec.corrspec,'DCC') == 1 || strcmp(CopulaSpec.corrspec,'TVC') == 1 || strcmp(CopulaSpec.corrspec,'ADCC') == 1 ...
                                || strcmp(CopulaSpec.corrspec,'GDCC') == 1
...
if strcmp(CopulaSpec.type,'SJC')
    ...
    if strcmp(dec,'CanonicalVine')==1
        for j=1:N-1
            for i=1:N-j
                if j==1
                    [Rt{j,i},likelihood(:,countl)]=CopulaLL_grm_1P_forecast(phi{j,i},[data(:,1),data(:,i+1)],CopulaSpec);
                    countl=countl+1;
                else
                    [Rt{j,i},likelihood(:,countl)]=CopulaLL_grm_1P_forecast(phi{j,i},[u{j-1,1},u{j-1,i+1}],CopulaSpec);
                    countl=countl+1;
                end
            end
            if j<N-1
    ...
    elseif strcmp(dec,'DVine')==1
        ...
        for i=1:N-1
            [Rt{1,i}]=CopulaLL_grm_1P_forecast(phi{1,i},[data(:,i) data(:,i+1)],CopulaSpec);
        end
        u{1,1}=hfunc_grm(data(:,1),data(:,2),phi{1,1},CopulaSpec);
        for k=1:N-3
            u{1,2*k}=hfunc_grm(data(:,k+2),data(:,k+1),phi{1,k+1},CopulaSpec);
        ...
        for j=2:(N-1)
            for i=1:(N-j)
                [Rt{j,i}]=CopulaLL_grm_1P_forecast(phi{j,i},[u{j-1,2*i-1} u{j-1,2*i}],CopulaSpec);
            end
            if j<N-1
                u{j,1}=hfunc_grm(u{j-1,1},u{j-1,2},phi{j,1}, CopulaSpec);
                if N>4
...
% -------------------------------------------------------------------------
% -------------------------------------------------------------------------
%  lese aus der Korrelationsmatrix jeweils den letzten Wert heraus, denn
%  nur dieser ist der "echte forecast"
c=N-1;
for i=1:N-1
    for j=1:c
        Rt_pred{i,j} = Rt{i,j}(end);
    end
    c=c-1;
end
