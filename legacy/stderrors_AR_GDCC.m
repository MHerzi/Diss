function [Tstatistic_all, stderrors] = stderrors_AR_GDCC(parameters, data,archP,garchQ,asymmG,dccP,dccQ,dccG,garchtype,errortype,GARCHOutput,const)
...
otherA = dcc_hessian_grm('AR_GDCC_full_likelihood', parameters, dccP*k+dccQ*k+dccG*k, data, archP, garchQ, asymmG, garchtype, errortype, dccP, dccQ, dccG, arlag, const);
...
for i=length(parameters)-dccP*k-k*dccQ+1:length(parameters)
    hparameters=parameters;
    hparameters(i)=hplus(i);
    [HOLDER, HOLDER1, indivlike] = AR_GDCC_full_likelihood(hparameters, data, archP, garchQ, asymmG, garchtype, errortype, dccP, dccQ, dccG, arlag, const);
