...
% Patton ersetzt
function [CopParam_tv, Weights_tv, tv_faktor, LL_Mix, LL, AIC, BIC, CopParam_1] = copulafitmix_tv_grm(family, data,Dynamic)
    ...
    switch family{i}
        ...
        case 't' % Skalarversion
            ...
            if strcmp(Dynamic,'ADCC')
                ...
                LB{i} = [1e-3; 1e-6; 1e-3; 2.1]';
                nonLinCon{i} = {'nonLinCon_Skalar_mix_tv_grm'}; % Funktion für nicht linearen Constraint
        ...
        case 'gaussian' % Skalarversion
            ...
            if strcmp(Dynamic,'ADCC')
                ...
                UB{i} = [0.9; 0.9; 0.99]';
                LB{i} = [1e-6; 1e-6; 1e-6]';
                nonLinCon{i} = {'nonLinCon_Skalar_mix_tv_grm'}; % Funktion für nicht lineare Constraint
...
[tv_faktor, likhood,exitflag,output,lambda,grad,hessian] =...
    fmincon('copulaLL_mix_tv_grm',x0_total,A,b,[],[],LB_total,UB_total,nonLinCon_total,options, CopParam_1, weights_1, family, data, P_total, Q_total, Dynamic);
[LL_Mix, LL, CopParam_tv, Weights_tv, density, likelihoods_mix, likelihoods] = copulaLL_mix_tv_grm(tv_faktor, CopParam_1, weights_1, family, data, P_total, Q_total, Dynamic);
