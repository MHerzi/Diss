function [result,autoLag,mu,H]=archtest_mean_parcorr(daten,nlags);
...
if k>1
    error('als Input darf nur ein Spaltenvektor verwendet werden!');
end

[PACF, Lags, Bounds] = parcorr(daten, nlags, [], []);

PACF=PACF(2:end,:);
m=1;
autoLag=0;
for i=1:nlags
    if abs(PACF(i,:))>=Bounds(1,1);
        autoLag(m)=i;
        m=m+1;
    end
end
...
else 'Residuen sind white noise'
end
