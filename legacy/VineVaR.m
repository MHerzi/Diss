function [Output,rnd_ret2] = VineVaR(Sim,GARCHOutput,GARCHOutput_Backtest,uniforecastP,n,MeanForecast,VarianceForecast,PrtFig,daten)

% Function that estimates VaR and Christoffersen's HIT-ratios for
% CopulaVine Functions
%
% USAGE:  [Output,rnd_ret] =
% VineVaR(Sim,GARCHOutput,GARCHOutput_Backtest,CopulaSpec,n,MeanForecast,Va
% rianceForecast,PrtFig,daten)
%
% INPUT:
...
for j = 1:t
    ...
    for i=1:k
        ...
        elseif strcmp(GARCHOutput{i}.dist,'STUDENTST') == 1
            rnd_ret{1,i}(:,j) = MeanForecast(j,i) + sqrt(VarianceForecast(j,i))*tinv(Sim{j}(:,i),GARCHOutput_Backtest{h}{i}.ParamsGARCH(end));
        elseif strcmp(GARCHOutput{i}.dist,'GED') == 1
            rnd_ret{1,i}(:,j) = MeanForecast(j,i) + sqrt(VarianceForecast(j,i))*gedinv_grm(Sim{j}(:,i),GARCHOutput_Backtest{h}{i}.ParamsGARCH(end));
        elseif strcmp(GARCHOutput{i}.dist,'SKEWT') == 1
            rnd_ret{1,i}(:,j) = MeanForecast(j,i) + sqrt(VarianceForecast(j,i))*skewtdis_inv_grm(Sim{j}(:,i),GARCHOutput_Backtest{h}{i}.ParamsGARCH(end-1),GARCHOutput_Backtest{1}{i}.ParamsGARCH(end));
        end
...
%         lösche Zahlen aus Sim, um Speicherplatz zu schaffen
        Sim{j}(:,i) = zeros(n,1);
    end
end
...
for i=1:k
    true_PuL = true_PuL + 1/k*exp(daten(end-t+1:end,i)); %für den einperiodigen Forecast muss eine Beobachtung abgezogen werden, da der letze Return nicht mehr zum Forecast benutz werden kann
end
