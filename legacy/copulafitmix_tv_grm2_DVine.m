% Im Gegensatz zur Original-Spezifikation werden hier die
% Optimierungsconstraints anders gesetzt
% 
...
% Patton ersetzt
function [CopParam_tv, Weights_tv, tv_faktor, LL_Mix, LL, AIC, BIC, CopParam_1, SE] = copulafitmix_tv_grm2_DVine(family, data, Dynamic, Cop_Stat, PrtFig)
...
%                 if s2==2 % f�r bivariate Zeitreihen sch�tze MVGARCH f�r Startparameters
%                     trdata = tinv(data,x0_DoF);
%                     x0{i} = Est_ADCC(trdata,1,1);
%                     x0{i} = [x0{i}' x0_DoF];
%                 else
%                     x0{i} = [0.1 0.01 0.8 x0_DoF];
%                 end
                x0{i} = [0.1 0.01 0.8 x0_DoF];
...
%                     x0{i} = [x0{i}' x0_DoF];
%                 else
%                     x0{i} = [ones(1,s2)*0.1 ones(1,s2)*0.8 x0_DoF];
%                 end
                x0{i} = [0.1 0.1 0.8 0.8 x0_DoF];
...
%                     x0{i} = [x0{i}' x0_DoF];
%                 else
%                     x0{i} = [ones(1,s2)*0.1 ones(1,s2)*0.01 ones(1,s2)*0.8 x0_DoF];
%                 end
                x0{i} = [0.1 0.1 0.01 0.01 0.8 0.8 x0_DoF];
...
%     'TolCon',10^-8,'TolFun',10^-6,'TolX',10^-8, 'AlwaysHonorConstraints', 'bounds', 'InitBarrierParam', 0.1);
options = optimset(options, 'Algorithm','trust-region-reflective','Display','iter','MaxIter',500,'MaxFunEvals',10000,...
    'TolCon',10^-6,'TolFun',10^-2,'TolX',10^-6, 'AlwaysHonorConstraints', 'bounds', 'InitBarrierParam', 0.3);
...
[LL_Mix, LL, CopParam_tv, Weights_tv, density] = copulaLL_mix_tv_grm2(tv_faktor, CopParam_1, family, data, P_total, Q_total, Dynamic, Density_Func, epsilon);
...
[Cov_Mat SE_robust p_Wert t_Wert] = copulaStat_mix_tv_2(Cop_Stat, tv_faktor, CopParam_1, weights_1, family, data, P_total, Q_total, Dynamic);
SE.CovMat=Cov_Mat;
