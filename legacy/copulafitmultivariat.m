...
function [Output LL exitflag numParam] = copulafitmultivariat(family, data, CopStat);
...
if not(strcmp(family, {'t' 'gaussian' 'clayton' 'gumbel' 'frank'}))
    error('die eingegebene Copula kann nicht verwendet werden');
end

% Prüfen dass bei archimedischen Copulas max 10 Zeitreihen im Datenset sind
if strcmp(family, {'clayton' 'gumbel' 'frank'})
    if s2>10
        error('archimedische Copulas sind auf 10 Zeitreihen begrenzt');
    end
end
...
switch family
    ...
    case {'clayton' 'frank' 'gumbel'} % archimedische Copulas
...
clear v v1 v2 v3 v4 v5 v6 v7 v8 v9 v10 % Max. Variablen = 10
v = [];
for i = 1:s2
...
clear v v1 v2 v3 v4 v5 v6 v7 v8 v9 v10
...
switch family
    case 't'
        ...
        CopulaDensity = copulapdfmultivariat(family, data, Rho, DoF); 
    case 'gaussian'
        CF = Coeff; % Cholesky Faktoren auslesen. 
        Rho = chol2corr(s2, CF); % Berechnen der Korrelationsmatrix via Cholesky Faktoren 
        CopulaDensity = copulapdfmultivariat(family, data, Rho);
end
% neg. LL berechnen
LL = -sum(log(CopulaDensity), 1);
end
