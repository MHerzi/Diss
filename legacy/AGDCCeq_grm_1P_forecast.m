function [Rt, veclRt]=AGDCCeq_grm_1P_forecast(theta,data,optimizer)
...
Q=1;
A = diag(theta(1:P*k));
G = diag(theta(P*k+1:2*P*k));
B = diag(theta(2*P*k+1:(2*P+Q)*k));
...
negativedata = (data<0).*data;
Nbar = cov(negativedata);
...
data = [zeros(m,k);data];
for j=(m+1):t+m+1
    Qt(:,:,j) = Qbar - A'*Qbar*A - B'*Qbar*B - G'*Nbar*G;
    for i = 1:P
        Qt(:,:,j) = Qt(:,:,j) + A'*(data(j-i,:)'*data(j-i,:))*A;
        Qt(:,:,j) = Qt(:,:,j) + G'*(negativedata(j-i,:)'*negativedata(j-i,:))*G;
    end
    for i=1:Q
