function [Output,rnd_ret2,CopParam_tv_back, Weights_tv_back, tv_faktor_back, CopParam_pred, weights_pred] = Backtest_VaR_Dynamic_Mixture(family,data,daten,GARCHOutput,GARCHOutput_Backtest,MeanForecast,VarianceForecast,PrtFig,Spec)
...
for i=start:forecastP:t-forecastP
    [CopParam_tv_back{count}, Weights_tv_back{count}, tv_faktor_back{count}, LL_Mix_back{count}, LL_back{count}, AIC_back{count}, BIC_back{count}] = copulafitmix_tv_grm2(family, data(1:i,:),Dynamic,'aus','on');
    ...
    for jj=1:forecastP
        ...
        if size(Spec.family,2)>1
            if CopParam_predSim{count}{jj}{2}>50
                CopParam_predSim{count}{jj}{2} = 50;
            else
        ...
        for ii=1:k
            ...
            if strcmp(GARCHOutput{ii}.dist,'GAUSS') == 1
                ...
                rnd_ret(:,ii)= MeanForecast(jj,ii) + sqrt(VarianceForecast(jj,ii))*norminv(Sim(:,ii));
            elseif strcmp(GARCHOutput{ii}.dist,'STUDENTST') == 1
                rnd_ret(:,ii) = MeanForecast(jj,ii) + sqrt(VarianceForecast(jj,ii))*tinv(Sim(:,ii),GARCHOutput_Backtest{g}{ii}.ParamsGARCH(end));
            elseif strcmp(GARCHOutput{ii}.dist,'GED') == 1
                rnd_ret(:,ii) = MeanForecast(jj,ii) + sqrt(VarianceForecast(jj,ii))*gedinv_grm(Sim(:,ii),GARCHOutput_Backtest{g}{ii}.ParamsGARCH(end));
            elseif strcmp(GARCHOutput{ii}.dist,'SKEWT') == 1
                rnd_ret(:,ii) = MeanForecast(jj,ii) + sqrt(VarianceForecast(jj,ii))*skewtdis_inv_grm(Sim(:,ii),GARCHOutput_Backtest{g}{ii}.ParamsGARCH(end-1),GARCHOutput_Backtest{g}{ii}.ParamsGARCH(end));
            end
