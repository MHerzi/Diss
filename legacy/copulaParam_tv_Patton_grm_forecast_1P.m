...
function [CopParam_tv] = copulaParam_tv_Patton_grm_forecast_1P(family,tv_faktor,CopParam_1,data,funktion,horizon);
...
for n = 2:s1+horizon
    ...
    if n <= MALags % Zugreifen auf kürzeren Zeitrahmen als MALags für K-Means Berechnung
        theta = tv_faktor(1)+ tv_faktor(2)*CopParam_tv(n) + tv_faktor(3)*(mean(abs(data(1:n-1,1)-data(1:n-1,2))));
    else
        ...
        % werden würden
        theta = tv_faktor(1)+ tv_faktor(2)*CopParam_tv(n) + tv_faktor(3)*(mean(abs(data(n-10:n-1,1)-data(n-10:n-1,2))));
    end
    
    CopParam_tv(n) = 1/(1+exp(-theta));		% Logistic Transformation nach Patton (2006)
end
