...
function testLegacySpecMapsToNestedConfiguration(testCase)
    ...
    verifyEqual(testCase, config.model.kind, "multiCopula");
    verifyEqual(testCase, config.dependence.kind, "copula");
    verifyEqual(testCase, config.dependence.dynamic, "ADCC");
    verifyEqual(testCase, config.backtest.initialWindow, 10);
    verifyEqual(testCase, config.backtest.forecastHorizon, 3);
    verifyEqual(testCase, config.simulation.numPaths, 5000);
    verifyEqual(testCase, config.risk.probability, 0.95);
end
