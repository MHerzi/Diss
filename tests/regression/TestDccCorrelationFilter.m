...
function testNamespacedKernelMatchesCompatibilityKernel(testCase)
    [stdResiduals, qBar] = syntheticResiduals();
    conditionalVariances = 0.05 + abs(stdResiduals) / 10;
    identityMatrix = eye(size(stdResiduals, 2));
    archMatrices = identityMatrix * 0.12;
    ...
    [expectedLogL, expectedValid, expectedRt, expectedLikelihoods, ...
        expectedQt] = dccCorrelationFilter(stdResiduals, ...
        conditionalVariances, qBar, archMatrices, garchMatrices, ...
        emptyMatrices, emptyMatrices);
    [actualLogL, actualValid, actualRt, actualLikelihoods, actualQt] = ...
        diss.dependence.correlation.filter(stdResiduals, ...
        conditionalVariances, qBar, archMatrices, garchMatrices, ...
        emptyMatrices, emptyMatrices);

    verifyEqual(testCase, actualValid, expectedValid);
    verifyEqual(testCase, actualLogL, expectedLogL, 'AbsTol', 0);
    verifyEqual(testCase, actualRt, expectedRt, 'AbsTol', 0);
    verifyEqual(testCase, actualLikelihoods, expectedLikelihoods, ...
        'AbsTol', 0);
    verifyEqual(testCase, actualQt, expectedQt, 'AbsTol', 0);
end
