function [parameters, LLF, stderrors, robustSE, ht, scores, resid, likelihood, EXITFLAG] = ar_avgarch_garch_grm(data , p , q , errors, arlag, const, startingvals, options)
% PURPOSE:
...
%
% USAGE:
%     [parameters, likelihood, stderrors, robustSE, ht, scores, resid] = fattailed_garch(data , p , q , errors, startingvals, options)
%
% INPUTS:
...
%
%
% Author: artin Grziska based on a code of Kevin Sheppard
...
%AR = parameters_AR; % Modifikation am 04.04.2013; parameters_AR wird als Zeilenvektor ausgegeben
...
sumA =  [[-eye(1+p+q) zeros(1+p+q,arlag)];[0  ones(1,p)  ones(1,q) zeros(1,arlag)]];
...
if strcmp(errors,'STUDENTST')
    [n,M]=size(sumA);
    sumA = [sumA' zeros(M,1);[zeros(1,n) -1]]';
    sumB= [sumB; -2.1];
elseif strcmp(errors,'GED')
    [n,M]=size(sumA);
    sumA = [sumA' zeros(M,1);[zeros(1,n) -1]]';
...
if errortype == 1
    startingvals = [omega ; alpha ; beta; AR];
else
    startingvals = [omega ; alpha ; beta; AR; nu];
end


[parameters, LLF, EXITFLAG, OUTPUT, LAMBDA, GRAD] =  fmincon('ar_avgarch_garchlikelihood_grm', startingvals , sumA  , sumB ,[] , [] , [] , [],'ar_fattailedgarch_nonlincon',options, data, p , q, errortype, arlag, const);
[LLF, Holder, Holder, resid] = ar_avgarch_garchlikelihood_grm(parameters , data , p , q, errortype, arlag, const);
...
hess = hessian_2sided('ar_avgarch_garchlikelihood_grm',parameters,data,p,q,errortype, arlag, const);
[Holder, ht, likelihood]=ar_avgarch_garchlikelihood_grm(parameters,data,p,q,errortype, arlag, const);
...
if nargout > 4
    ...
    for i=1:length(parameters)
        ...
        [HOLDER, HOLDER1, indivlike] = ar_avgarch_garchlikelihood_grm(hparameters,data,p,q,errortype, arlag, const);
