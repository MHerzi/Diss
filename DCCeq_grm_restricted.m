function [Rt, veclRt]=DCCeq_grm_restricted(theta,data,optimizer)
...
% % restricted version for use with clayton and gumbel copula; both copulas
% are restricted to positive dependence: if negative the off diagional
% elements of Qt are set to 0
% Referencec: Heinen, Valdesogo (2009). Canonical Vine Autoregressive Model
...
% set off-diagonal elements to zero if negative
for j=(m+1):t+m
    if Qt(2,1,j)<0 %only bivariate dependence in vine copulas, so it's enough to check only one argument
            Qt(2,1,j) = 0;
            Qt(1,2,j) = 0;
    end
