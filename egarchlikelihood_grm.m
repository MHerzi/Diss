function [LLF, h, likelihoods]= egarchlikelihood_grm(parameters,data,p,o,q,T,stdEstimate,errortype);
% PURPOSE:
...
if errortype == 1
    ...
    data = data - x*b0;
    arlag = length(parameters)-(1+p+o+q+1);
    parameters = parameters(1:1+p+o+q);
else
    b0 = parameters(1+p+o+q+1:end-1);
    n = length(data);
    nlag = 1;
    x = [ones(n,1) mlag(data,nlag)];
    x = trimr(x,nlag,0);
    ...
    data = data - x*b0;
    arlag = length(parameters)-(1+p+o+q+1);
    parameters = [parameters(1:1+p+o+q); parameters(end)];
end
...
h=egarchcore_grm(data, parameters, stdEstimate, p, o, q ,m , T);
