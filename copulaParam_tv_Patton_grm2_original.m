...
function [CopParam_tv] = copulaParam_tv_Patton_grm2_original(family,tv_faktor,CopParam_1,data,funktion,horizon);
...
for n = 2:s1+horizon
    ...
    if n <= MALags % Zugreifen auf kürzeren Zeitrahmen als MALags für K-Means Berechnung
        psi(n) = K+ AR*psi(n-1) + MA*(mean(abs(u(1:n-1)-v(1:n-1))));
    elseif n > s1+1
        ...
        % werden würden
        psi(n) = mean(psi(n-MALags:n-1));
    else
        psi(n) = K+ AR*psi(n-1) + MA*(mean(abs(u(n-10:n-1)-v(n-10:n-1))));
    end
end
