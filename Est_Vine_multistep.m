function [StrOutput,phi] = Est_Vine_multistep(CopulaSpec,N,data,ParSize,T)
...
for i=1:N-1
    ...
    if exitflag{1,i} <0 || LogL{1,i} == 10E+7
        options = optimset('fmincon');
        options = optimset(options,'Display','iter','TolCon',10^-10,'TolFun',10^-10,'TolX',10^-10,'Algorithm','interior-point','FunValCheck','on','MaxFunEvals',10000);
        epsilon = 10^(-3);
        exitFlag = 0;
        count = 0;
