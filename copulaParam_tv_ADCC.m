...
function [R_tv] = copulaParam_tv_ADCC(tv_faktor, P, Q, data_inv, funktion, horizon);
...
% Faktoren des ADCC Modells übergeben
a = tv_faktor(1:P); % ARCH Faktor
g = tv_faktor(P+1:2*P); % Leverage Faktor
b = tv_faktor(2*P+1:2*P+Q); % GARCH Faktor
...
Nbar_negative = cov(data_inv.*(data_inv < 0)); % Kovarianzmatrix der negativen stand. Residuen
Qinitial = Qbar * (1 - a^2 - b^2) - Nbar_negative * g^2;
m = max(P,Q);
...
for j = (m+1):s1+m+horizon
    Qt(:,:,j) = Qinitial;
    for i=1:P
        Qt(:,:,j) = Qt(:,:,j) + a(i)*(stdresid(j-i,:)'*stdresid(j-i,:))*a(i); % ARCH Faktor
        Qt(:,:,j) = Qt(:,:,j) + g(i)*(negativey(j-i,:)'*negativey(j-i,:))*g(i); % Leverage Faktor
    end
    for i = 1:Q
        Qt(:,:,j) = Qt(:,:,j) + b(i)*Qt(:,:,j-i)*b(i); % GARCH Faktor
    end
