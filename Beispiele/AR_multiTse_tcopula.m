...
% Tse
[ kappa_Tse LL_Tse] = fmincon('multi_tCopula_tvp1_TseRt',theta0,A,b,[],[],lower,upper,[],options,[u1, u2, u3],k);% In dem Paper von Tse(2002) wir m = k gesetzt;
[LL_Tse Rt_Tse] = multi_tCopula_tvp1_TseRt(kappa_Tse, [u1, u2, u3],k);
% 
% Runden, auf die fünfte Nachkommastelle, da sonst auf der Hauptdiagonalen
% Elemente stehen die >1 sind
Rt_Tse = round(Rt_Tse*1e5)*1e-5; 


% Auf der Haupdiagonalen konvergiert Rt gegen 1. Damit auf der Hauptdiagonalen nur 1en stehen, lösche die (:,:,k)
% k-Elemente bei denen dies nicht der Fall ist
d = zeros(t,1);
for i=1:size(Rt_Tse,3)
    if sum(sum(diag(Rt_Tse(:,:,i)))) < k-1e-6
        d(i)=1;
    else d(i)=0;
    end
end
RtTse = Rt_Tse(:,:,sum(d)+1:end);
...
h=1;
for i=1:t
    random(:,h:(h+(k-1))) = copularnd('t',RtTse(:,:,i),kappa_Tse(end),N);
    h=h+k;
end
...
for j=1:k
    for i = 1:k:t*k
        crnd{j}(:,h) = tinv(random(:,i),kappa_Tse(end));
        h=h+1;
    end
    h=1;
end
