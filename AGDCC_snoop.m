function [Rt_pred,Qt_pred] = AGDCC_snoop(params, data, archP, garchQ, ...
...
asymmetricValues = reshape(params( ...
    asymmetricStart:asymmetricStart+archParameterCount-1), k, dccP);
garchStart = asymmetricStart + archParameterCount;
garchValues = reshape(params(garchStart:garchStart+dccQ*k-1), k, dccQ);
archMatrices = dccDiagonalMatrixSequence(archValues);
asymmetricMatrices = dccDiagonalMatrixSequence(asymmetricValues);
