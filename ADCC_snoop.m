function   [Rt_pred,Qt_pred] = ADCC_snoop(params, data, archP, garchQ, o, garchtype, errortype, dccP, dccQ)
...
a_negative = diag(params(index+(P):index+2*(P)-1));   % ARCH for negative residuals
b = diag(params(index+2*(P):index+2*(P)+(Q)-1));        % GARCH
...
negativeStdresid =[zeros(m,k); negativeStdresid];
for j = (m+1):t+m+1
   Qt(:,:,j) = Qinitial;
   for i=1:P
