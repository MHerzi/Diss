function transformeddata = ivecl_grm(data, type)
% PURPOSE:
...
% Author: Manthos Vogiatzoglou, UoM, 2008, Modification by Martin Grziska,
% Februar, 2nd, 2010
...
% Bestimme die Dimension der Output-Matrix (obere Dreiecksmatrix +
% Hauptsiagonale)
syms xx
sizeout = solve(xx*(xx-1)+2*xx-(2*k),xx);
sizeout = double(abs(sizeout(1,1)));

index=1;

if iscell(data)==1
    transformeddata = cell(sizeout);
    for i=1:sizeout
        for j=(i+1):sizeout
