function[comparestat]=asymmtest_chi(data)
...
criticalvalF=AssetVarianz_negative./AssetVarianz_positive;

for i=1:size(AssetVarianz_positive,2)
dofpositive(:,i)=size(AsymmReturnpositive_neu(i).result,1)-1;
dofnegative(:,i)=size(AsymmReturnnegative_neu(i).result,1)-1;
Fstat(:,i)=finv(0.95,dofnegative(:,i),dofpositive(:,i));
end

comparestat=[criticalvalF;Fstat];


DiffVarianz=AssetVarianz_negative-AssetVarianz_positive;
