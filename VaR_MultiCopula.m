function [Output,rnd_ret2] = VaR_MultiCopula(MeanForecast,VarianceForecast,GARCHOutput,Rt,n,daten,start,GARCHOutput_Backtest,Spec,params,PrtFig,maxarlag,U,uniforecastP)

% Berechnung des VaR für AGDCC-Modelle (auch copula mit AGDCC-Struktur)
%
% USAGE: [Output,rnd_ret2] = VaR_MultiGARCH(MeanForecast,VarianceForecast,GARCHOutput,Rt,n,daten,start)
%
% INPUTS:
...
%           uniforcastP : forecasting period for univariate models
%        
%
% OUTPUTS:
...
%
% Author: Martin Grziska, last maodification: 08/06/2010
...
jj = 1:size(GARCHOutput_Backtest,2); %Variable zählt die univariaten GARCH-Backtest Modelle mit;
h=1;
for j = 1:t
    if j==uniforecastP*jj(h)+1 % sorge dafür, dass immer die richtigen univariaten Backtest-Parameter genommen werden; forecastP ist auch der Zeitreum für ein univariates Backtest-Garch Modell sobald forecastP erreicht ist, kommt der nächste Backtest Zeitraum
        h=h+1;
    end
    for i=1:k
        if strcmp(GARCHOutput{i}.dist,'GAUSS') == 1
            rnd_ret{1,i}(:,j) = MeanForecast(j,i) + sqrt(VarianceForecast(j,i))*norminv(U{j}(:,i));
        elseif strcmp(GARCHOutput{i}.dist,'STUDENTST') == 1
            rnd_ret{1,i}(:,j) = MeanForecast(j,i) + sqrt(VarianceForecast(j,i))*tinv(U{j}(:,i),GARCHOutput_Backtest{h}{i}.ParamsGARCH(end));
        elseif strcmp(GARCHOutput{i}.dist,'GED') == 1
            rnd_ret{1,i}(:,j) = MeanForecast(j,i) + sqrt(VarianceForecast(j,i))*gedinv_grm(U{j}(:,i),GARCHOutput_Backtest{h}{i}.ParamsGARCH(end));
        elseif strcmp(GARCHOutput{i}.dist,'SKEWT') == 1
            rnd_ret{1,i}(:,j) = MeanForecast(j,i) + sqrt(VarianceForecast(j,i))*skewtdis_inv_grm(U{j}(:,i),GARCHOutput_Backtest{h}{i}.ParamsGARCH(end-1),GARCHOutput_Backtest{h}{i}.ParamsGARCH(end));
        end
%         nicht sortierte returns für die Nutzenfunktion
        rnd_ret2{1,i}(:,j) = rnd_ret{1,i}(:,j);
...
% % Es gehen Beobachtungen durch die AR-Schätzung verloren
true_PuL=0;
for i=1:k
    true_PuL = true_PuL + 1/k*exp(daten(end-t+1:end,i));
end
