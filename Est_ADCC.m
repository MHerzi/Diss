function [adccparameters, negativeLikelihood, exitFlag] = Est_ADCC(stdresid, dccP, dccQ)
...
options  =  optimset(options , 'TolCon'      , epsilon);
options  =  optimset(options , 'TolX'      , epsilon);
options  =  optimset(options , 'Algorithm'   ,'active-set');
    if epsilon < 10^(-3)
        disp(['epsilon = ', num2str(epsilon)])
    end
    [adccparameters, negativeLikelihood, exitFlag] = fmincon('ADCC_likelihood_grm', adccstarting, A, b, ...
