function [Output,rnd_ret2,CopParam_tv_back, Weights_tv_back, tv_faktor_back, CopParam_pred, weights_pred] = Backtest_VaR_Dynamic_Mixture_multistep(family,data,daten,GARCHOutput,GARCHOutput_Backtest,MeanForecast,VarianceForecast,PrtFig,Spec,upareto)

% Berechnung des Backtest-VaR für multivariate Mixture-Copula Modelle
%
% INPUTS:
%        family:          string('gaussian','t','clayton','gumbel') beachte:
%                         elliptische Copulas stehen immer an erster Stelle
%        data:            unif(0,1) verteilte Daten - die Marginalverteilung
%        daten:           return-matrix
%        start:           letzter Zeitpunkt für
%        numbSim:         # der zu simulierenden unif(0,1) Variablen für den
%                         Monte-Carlo VaR
%        GARCHOutput:     Structure Variable für die Marginalverteilungen
%        GARCHOutput_add: Structure Variable für die Backtest-Marginalverteilungen
%        Dynamic:         Dynamik-Spezifikation für elliptische copual in der
%                         Mixture-Copula
%        PrtFig:          string ('on','off') um VaR zu printen
%        maxarlag:        maximale Anzahl der AR-Lags bei Schätzung der
%                         univariaten GARCH
%        const:           1 wenn univariate GARCH mit Konstante geschätzt wurden, sont 0
%        ardiff:          Differenz zwischen dem kleinsten und größten AR-lag der univariaten GARCH
%        forecastP:       Vorhersagehorizont der univariaten GARCH-Modelle
%
% OUTPUTS:
%        Output:          Structure die den VaR und verschiedene Tests desselben
%                         enthält
% Author:
%                         Martin Grziska,
% last modification: 08/01/2010

% Berechnung der Koeffizienten des Backtest-sample
[t,k]= size(data);

% einperiodige forecasts
horizon = 1;

% bestimme die Anzahl der Copulas
nc = size(family,2);

% family muss als cell soezifiziert sein
if ~iscell(family)
    error('Die Variable "family" muss als cell-Variable spzifiziert sein')
end
...
% kreieren von verschiedenen Variablen, um Matlab-speed zu erhöhen
rnd_ret2 = cell(1,k);
VaR991 = cell(1,k);
for i=1:k
...
for i=start:forecastP:t-forecastP
    [CopParam_tv_back{count}, Weights_tv_back{count}, tv_faktor_back{count}, LL_Mix_back{count}, LL_back{count}, AIC_back{count}, BIC_back{count}] = copulafitmix_tv_grm2_var(family, data(1:i,:),Dynamic,'aus','on');
    h=0; %Foecast geht von 0 bis forecastP-1; letzte Return-forecast könnte nicht mehr mit PuL verglichen werden
    for jj=1:forecastP
        [CopParam_pred{count}{jj}, weights_pred{count}{jj}] = copulamix_tv_paramforecast_2_grm(tv_faktor_back{count}, nc, family, data(1:i+h,:), Spec.archP, Spec.garchQ, horizon, Dynamic);
        %         wenn der Copula-parameter >50 ist liefert der Algorithmus
        %         falsche Werte
        CopParam_predSim{count}{jj}=CopParam_pred{count}{jj}; %behalte die Originalwerte und kreiere neue Variable in der die neuen Werte stehen, wenn die alen ersetzt wurden
        %         für Mixture-Copulas restringiere den copula-Parameter
        if size(Spec.family,2)>1
            if CopParam_predSim{count}{jj}{2}>20
                CopParam_predSim{count}{jj}{2} = 20;
            else
                CopParam_predSim{count}{jj}{2}=CopParam_pred{count}{jj}{2};
            end
        end
        Sim = copulamixrnd(family, CopParam_predSim{count}{jj}, weights_pred{count}{jj}, k, n);
        h=h+1;
        ...
        for ii=1:k
            rnd_ret=zeros(n,k);
            if strcmp(Spec.tails,'pareto') %wenn unif(0,1)-Variablen durch paretotail bestimmt wurden, muss inverse cdf der paretotail auf unif(0,1)-Variablen angewandt werden
                if strcmp(GARCHOutput{ii}.dist,'GAUSS') == 1
                    rnd_ret(:,ii)= MeanForecast(countret,ii) + sqrt(VarianceForecast(countret,ii))*upareto{ii}.icdf(Sim(:,ii));
                elseif strcmp(GARCHOutput{ii}.dist,'STUDENTST') == 1
                    rnd_ret(:,ii) = MeanForecast(countret,ii) + sqrt(VarianceForecast(countret,ii))*upareto{ii}.icdf(Sim(:,ii));
                elseif strcmp(GARCHOutput{ii}.dist,'GED') == 1
                    rnd_ret(:,ii) = MeanForecast(countret,ii) + sqrt(VarianceForecast(countret,ii))*upareto{ii}.icdf(Sim(:,ii));
                elseif strcmp(GARCHOutput{ii}.dist,'SKEWT') == 1
                    rnd_ret(:,ii) = MeanForecast(countret,ii) + sqrt(VarianceForecast(countret,ii))*upareto{ii}.icdf(Sim(:,ii));
                end
            else
                if strcmp(GARCHOutput{ii}.dist,'GAUSS') == 1
                ...
                elseif strcmp(GARCHOutput{ii}.dist,'STUDENTST') == 1
                    rnd_ret(:,ii) = MeanForecast(countret,ii) + sqrt(VarianceForecast(countret,ii))*tinv(Sim(:,ii),GARCHOutput_Backtest{g}{ii}.ParamsGARCH(end));
                elseif strcmp(GARCHOutput{ii}.dist,'GED') == 1
                    rnd_ret(:,ii) = MeanForecast(countret,ii) + sqrt(VarianceForecast(countret,ii))*gedinv_grm(Sim(:,ii),GARCHOutput_Backtest{g}{ii}.ParamsGARCH(end));
                elseif strcmp(GARCHOutput{ii}.dist,'SKEWT') == 1
                    rnd_ret(:,ii) = MeanForecast(countret,ii) + sqrt(VarianceForecast(countret,ii))*skewtdis_inv_grm(Sim(:,ii),GARCHOutput_Backtest{g}{ii}.ParamsGARCH(end-1),GARCHOutput_Backtest{g}{ii}.ParamsGARCH(end));
                end
            end
