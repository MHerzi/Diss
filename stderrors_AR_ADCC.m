function [Tstatistic_all, stderrors] = stderrors_AR_ADCC(parameters,data,archP,garchQ,asymmG,dccP,dccQ,dccG,garchtype,errortype,GARCHOutput,const)
...
index=1;
for i=1:k
    workingsize=size(GARCHOutput{i}.stderrors,1);
    A(index:index+workingsize-1,index:index+workingsize-1)=GARCHOutput{i}.stderrors^(-1);
    index=index+workingsize;
end
otherA = dcc_hessian_grm('AR_ADCC_full_likelihood', parameters, dccP+dccQ+dccG, data, archP, garchQ, asymmG, garchtype, errortype, dccP, dccQ, dccG, arlag, const);
...
for i=length(parameters)-dccP-dccQ-dccG+1:length(parameters)
    hparameters=parameters;
    hparameters(i)=hplus(i);
    [HOLDER, HOLDER1, indivlike] = AR_ADCC_full_likelihood(hparameters, data, archP, garchQ, asymmG, garchtype, errortype, dccP, dccQ, dccG, arlag, const);
