function [Rt_pred,GARCHOutput_add,dccparameters_backtest] = BacktestMultiGARCH_multistep(stdresid,start,daten,GARCHOutput,forecastP,Spec,archP,garchQ,GARCHOutput_add)
...
% parameters_add=cell(size(GARCHOutput_add,2),1);
...
%     [AR_pred_all{count},ht_pred_all{count}, resid_all{count}] = ar_univariate_snoop_forecast1P(daten(1:i+maxarlag+forecastP,:),GARCHOutput_add{count},const); %zäle maxarlag dazu da innerhalb der Funktion selber noch einmal um AR-Laglängen korrigiert wird
...
%     end
    ...
    for l=1:k
        parameters_add{count}(h:h+size(GARCHOutput_add{count}{l}.ParamsGARCH,1)-1,1) = GARCHOutput_add{count}{l}.ParamsGARCH;
        h = h + size(GARCHOutput_add{count}{l}.ParamsGARCH,1);
    end
