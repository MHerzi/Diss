% function [Output] = VaR_tv_Mixture(MeanForecast,VarianceForecast,GARCHOutput,n,daten,start,GARCHOutput_add,Spec,params,PrtFig)

% Berechnung des VaR für tv_Mixture-Modelle mit Simulationsdaten von
% Valentin
%
...
for j = 1:t
    for i=1:k
        ...
        elseif strcmp(GARCHOutput{i}.dist,'STUDENTST') == 1
            rnd_ret{1,i}(:,j) = MeanForecast(j,i) + sqrt(VarianceForecast(j,i))*tinv(U{j}(:,i),GARCHOutput_add{1}{i}.ParamsGARCH(end));
        elseif strcmp(GARCHOutput{i}.dist,'GED') == 1
            rnd_ret{1,i}(:,j) = MeanForecast(j,i) + sqrt(VarianceForecast(j,i))*gedinv_grm(U{j}(:,i),GARCHOutput_add{1}{i}.ParamsGARCH(end));
        elseif strcmp(GARCHOutput{i}.dist,'SKEWT') == 1
            rnd_ret{1,i}(:,j) = MeanForecast(j,i) + sqrt(VarianceForecast(j,i))*skewtdis_inv(U{j}(:,i),GARCHOutput_add{1}{i}.ParamsGARCH(end-1),GARCHOutput_add{1}{i}.ParamsGARCH(end));
        end
...
% die Variablen habe durch die Simulation der U's schon die gewünschte
