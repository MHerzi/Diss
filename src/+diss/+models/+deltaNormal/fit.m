function [model, state] = fit(returns, config, ~, context)
...
% CONTEXT is accepted to satisfy the common adapter contract.
if isfield(context, 'Window') && ~isscalar(context.Window)
    error('diss:deltaNormal:InvalidContext', ...
        'Context.Window must be scalar when supplied.');
end
