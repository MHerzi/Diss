function [results, finalState] = run(data, config, initialState)
%RUN Execute one reproducible dissertation experiment.
%
...
switch config.execution.mode
    case "full"
        [results, finalState] = runFullSample(plan, initialState);
    case "backtest"
        [results, finalState] = runBacktest(plan, initialState);
    otherwise
...
results.SchemaVersion = config.schemaVersion;
results.Evaluation = diss.evaluation.summarize(results);
results.Manifest = diss.experiment.createManifest(plan, results.Metadata);

end

function [results, state] = runFullSample(plan, state)
reuseMarginal = isfield(state, 'MarginalModels') && ...
    isfield(state, 'MarginalSpecifications');
context = struct('RefitMarginal', ~reuseMarginal, 'Window', 1);
[model, state] = diss.fitWindow( ...
    plan.Data.Returns, plan.Config, state, context);
...
function [results, state] = runBacktest(plan, state)
...
for windowIndex = 1:height(schedule)
    ...
    if config.output.saveWindowModels
        windowModels{windowIndex} = diss.experiment.compactModel( ...
            model, config.output.saveTrainingData);
    end
end
